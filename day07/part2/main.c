...
typedef struct DirectoryCandidate {
    Directory *directory;
    long size;
} DirectoryCandidate;

DirectoryCandidate *solve_problem(Directory* directory, long amount_of_space_to_free, DirectoryCandidate *currentCandidate) {
    LinkedListElement *element = directory->subdirectories->head;
    while (element) {
        Directory *subdirectory = (Directory*) element->element;
        currentCandidate = solve_problem(subdirectory, amount_of_space_to_free, currentCandidate);
        element = element->next;
    }

    long size = directory_size(directory);
    if (size >= amount_of_space_to_free) {
        if (currentCandidate) {
            if (size < currentCandidate->size) {
                currentCandidate->directory = directory;
                currentCandidate->size = size;
            }
        } else {
            currentCandidate = calloc(1, sizeof(DirectoryCandidate));
            currentCandidate->directory = directory;
            currentCandidate->size = size;
        }
    }
...
//    print_directory(root, 0);
    long total_disk_space = 70000000;
    long needed_space = 30000000;
    long used_disk_space = directory_size(root);
    long free_disk_space = total_disk_space - used_disk_space;
    long amount_of_space_to_free = needed_space - free_disk_space;

    DirectoryCandidate *deletionCandidate = solve_problem(root, amount_of_space_to_free, NULL);

    printf("%d\n", deletionCandidate->size);

    free(deletionCandidate);
    free_directory(root);
